...
async function getLastRecordInDB() {
  let client;
  try {
    client = await pool.connect();
    const query = "SELECT * FROM audit ORDER BY event_timestamp DESC LIMIT 1";
    const { rows } = await client.query(query);
    return rows[0];
  } catch (error) {
    console.error(
      "Error retrieving last record from PostgreSQL database:",
      error
    );
    return null;
  } finally {
...
app.get("/api/viewData", async (req, res) => {
  try {
    ...
    if (result.rows) {
      ...
      const lastRecordInDB = await getLastRecordInDB(); // Function to get the last record in the database
      if (lastRecordInDB && response.length > 0) {
        const lastEventTimestampInDB = new Date(lastRecordInDB.event_timestamp);
        const lastEventTimestampInViewData = new Date(
          response[response.length - 1].event_timestamp
        );
        if (lastEventTimestampInViewData > lastEventTimestampInDB) {
          // Insert fetched data into PostgreSQL database
          await insertDataIntoPostgres(response);
